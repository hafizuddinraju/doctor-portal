...
import { saveUserMongodb } from '../../FunctionAll/Function';
import { AuthDataContext } from '../../UseContext/AuthContext';

const Signup = () => {
    const {CreateSignUp,profileUpdate,setSign,googleLogin,githubLogin,user} = useContext(AuthDataContext);
    ...
    const handleGithub = ()=>{
        ...
        .then(result =>{
            const user = result.user;
            console.log(user);
            toast.success('Git Hub Login', {autoClose:500})
            navigate('/')
        })
        .catch(error =>{
            toast.error(`${error}`,{autoClose:500})
  ...
  <form onSubmit={handleSubmit(handleSignUp)}  className="space-y-6 ng-untouched ng-pristine ng-valid">
      ...
      <button type='submit' className="block w-full font-semibold p-3 text-center rounded-sm text-gray-900 bg-sky-400">Sign up</button>
  </form>
  ...
  <p className="text-xs text-center sm:px-6 text-gray-400">All ready have an account?
      <Link rel="noopener noreferrer" to='/login' className="underline text-sky-700">Log In</Link>
  </p>
</div>
      </div>
    );
